...
const DB_VERSION = 4 // 升级版本号，移除 spriteTasks
...
export function openDB(): Promise<void> {
  ...
  dbReadyPromise = new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (oldVersion < 3) {
        // 版本 3: 之前添加过雪碧图任务存储（现在不需要了）
      }

      if (oldVersion < 4) {
        // 版本 4: 删除雪碧图任务存储
        if (database.objectStoreNames.contains('spriteTasks')) {
          database.deleteObjectStore('spriteTasks')
          console.log('IndexedDB 雪碧图任务存储空间已删除')
        }
      }
    }
  })
...
export async function migrateFromLocalStorage(): Promise<number> {
  ...
  try {
    ...
    // 迁移成功后删除 localStorage 里的旧数据，避免重复迁移
